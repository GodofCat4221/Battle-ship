...
def ai_pass():
    ...
    while ai_guessing:
        ...
        if player.board[ai_guess_y][ai_guess_x] == s_ship:
            ...
            if destroy:
                print("\nВраг уничтожил ваше судно (X: %s, Y: %s)." % (ai_guess_x, ai_guess_y))
            else:
                print("\nВраг повредил ваше судно (X: %s, Y: %s)." % (ai_guess_x, ai_guess_y))
            break
            ...
            print("\nВраг выстрелил мимо (X: %s, Y: %s)." % (ai_guess_x, ai_guess_y))
            break
...
while game:
    ...
    if len(player.spawned) == 0:
        print("Вы проиграли!")
        input("Осталось кораблей: " + str(len(ai.spawned)) + ".")
        break
